	...
	public static void spreadAnimals() {
		...
		for(int x = 0; x != World.getWorld().size.getWidth(); x+=16) {
			if(SquareCraft.ran.nextBoolean()) {
				...
				for(int i = 0; i != a; i++)World.getWorld().spawnEntity(new Pig(new Location(World.getWorld(), (x+SquareCraft.randomInteger(0, 16))*SquareCraft.blocksize, 0)));
			}
		}
		for(int x = 0; x != World.getWorld().size.getWidth(); x+=16) {
			if(SquareCraft.ran.nextBoolean()) {
				a = SquareCraft.randomInteger(0, max_chicken_in_chunk);
				for(int i = 0; i != a; i++)World.getWorld().spawnEntity(new Chicken(new Location(World.getWorld(), (x+SquareCraft.randomInteger(0, 16))*SquareCraft.blocksize, 0)));
			}
		}
	}
