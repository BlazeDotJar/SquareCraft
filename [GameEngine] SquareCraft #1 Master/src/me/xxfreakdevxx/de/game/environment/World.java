...
//							System.out.println("Chunk ID: "+(x/ChunkManager.chunksizeBlocks)+" Chunks: "+ChunkManager.getChunks().size());
							...
							highest_stone = ChunkManager.getChunk(x/ChunkManager.chunksizeBlocks).getHighestBlockAt(x, Material.GRASS);
							highest_y = highest_stone.getLocation().getIntY(false);
...
//						System.out.println("Claimed: "+loc.getLocationString());
			...
			public Block getHighestBlockAt(int x) {
				...
				for(int i = 0; i != world.size.worldheight; i++) {
					if(blocks.contains(x+":"+(i*blocksize)) && blocks.get(x+":"+(i*blocksize)).getMaterial() != Material.AIR) {
